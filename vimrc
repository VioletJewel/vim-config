...
if !has('nvim') && !has('gui_running')
  ...
  for ord in range(33,61)+range(63,90)+[92]+range(94,126)
    let char = ord == 34 ? '\"' : ord == 124 ? '\|' : nr2char(ord)
...
cabbrev <expr> vsb getcmdline() =~# '^vsb' && getcmdpos() == 4 ? 'vert sb' : 'vsb'
cabbrev <expr> vh getcmdline() =~# '^vh' && getcmdpos() == 3 ? 'vert h' : 'vh'
cabbrev <expr> vsn getcmdline() =~# '^vsn' && getcmdpos() == 4 ? 'vert sn' : 'vsn'
" cabbrev <expr> vsp getcmdline() =~# '^vsp' && getcmdpos() == 4 ? 'vert sp' : 'vsp'
cabbrev <expr> vsbn getcmdline() =~# '^vsbn' && getcmdpos() == 5 ? 'vert sbn' : 'vsbn'
cabbrev <expr> vsbp getcmdline() =~# '^vsbp' && getcmdpos() == 5 ? 'vert sbp' : 'vsbp'
cabbrev <expr> vterm getcmdline() =~# '^vterm' && getcmdpos() == 6 ? 'vert term' : 'vterm'
...
cnoremap <expr> <c-t> getcmdtype() =~ '[/?]' ? (foldclosed('.') == -1 ? '<c-t>' : '').'<c-r>=execute("norm! zxzz")[-1]<cr>' : ''
cnoremap <expr> <c-g> getcmdtype() =~ '[/?]' ? (foldclosed('.') == -1 ? '<c-g>' : '').'<c-r>=execute("norm! zxzz")[-1]<cr>' : ''
...
" Swap foo\ bar qux\ baz
function! s:swap(l1, l2, s1, s2)
  let [s1, s2] = [a:s1->escape('/\'), a:s2->escape('/\')]
  execute printf('%d,%ds/\V\(%s\|%s\)/\={%s:%s,%s:%s}[submatch(0)]/g', a:l1, a:l2, s1, s2, string(a:s1), string(a:s2), string(a:s2), string(a:s1))
endfunction
command! -nargs=+ -range=% Swap call <sid>swap(<line1>, <line2>, <f-args>)
...
if !filereadable($VIMDIR.'/autoload/plug.vim')
  ...
  function! s:plugWarnSetup()
    echohl WarningMsg
    if !executable('curl')
      unsilent echom 'WARNING: curl is not installed; install it, then run :PlugDownload'
    else
      unsilent echom 'WARNING: run :PlugDownload to install vim-plug'
    endif
    echohl None
